...
def plan_couverture(request):
    return render(request, 'btssizing_app/plan_couverture.html')
...
def add_plan_couverture(request):
    ...
    if step == 1 : 
        ...
        p_c = Plan_couverture.objects.get(id=data['id'])
        p_c.projet = Projet.objects.get(id=data['project_id'])
        ...
        return render(
            ...
            context={'step':2, 'step_name':'Finalisation de la planification',
                'projects':projects, 'id':plan_couverture.id
            }
        )
    elif step == 2 :
        data = request.POST
        ...
        if p_c.modele_propagation == "PEL" :
            ...
            r = pow(10, ( p_c.pl - 32.4 - 20*math.log(p_c.f,10) )/20 ) 
        elif p_c.modele_propagation == "MOH" :
            # Modèle Okumura-Hata
            r = pow(10, (p_c.pl - 69.55 - 26.16*math.log(p_c.f) + p_c.ahm + 4.78*((math.log(p_c.f, 10))**2) - 18.33*math.log(p_c.f,10) + 35.94)/(44.9 - 6.55*math.log(p_c.hb, 10)) )
        elif p_c.modele_propagation == "MCH" :
            # Modèle Cost231-Hata
            r = pow(10, (p_c.pl - 46.3 - 33.9*math.log(p_c.f,10) + 13.82*math.log(p_c.hb, 10) + p_c.ahm - p_c.cm )/(44.9 - 6.55*math.log(p_c.hb, 10)) )
            ...
            r = pow(10, (p_c.pl - k1 - k3*p_c.hm - k4*math.log(p_c.hm) - k5*math.log(p_c.heff) - k7 - kclutter)/(k2+ k6*math.log(p_c.heff, 10)))
        ...
        # 7 : Nombre de BTS
        projet = Projet.objects.get(id=p_c.projet)
